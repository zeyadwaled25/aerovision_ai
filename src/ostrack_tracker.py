"""
📌 OSTrack Boss Engine — The Apex Self-Aware Edition
✅ Base: Stable Working Code (GPU/CPU Safe)
✅ Architecture: Deep State Machine, Score History Window, Adaptive Thresholds,
Progressive Search Expansion, Direction Penalty, & Motion-Scale Logic.
"""
import sys
import torch
import cv2
    ...
    with torch.no_grad():
        while True:
            ...
            if len(score_history) > 5:
                score_history.pop(0)
            stable_score = np.mean(score_history) if score_history else raw_score
            ...
            # Search Expansion when LOST
            if state == "LOST":
                allowed_jump *= (1 + lost_counter * 0.1)
            ...
            # 🤖 إعادة تقييم الحالة (State) بعد الغرامات
            if state != "LOST":
                if effective_score > track_th:
                    state = "TRACKING"
                elif effective_score > uncertain_th:
